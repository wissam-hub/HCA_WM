...
st.set_page_config(layout="wide", page_icon = "pics/smok_animation.jpg", page_title = "Smoking Trend Analysis")
...
def risk_factors():
    ...
    risk_factors = [title.split('Risk:')[1].split(' - Sex:')[0].strip() for title in data.columns[3:]]
    ...
    entity_dropdown = col1.selectbox('Entity:', ["World", "Middle East & North Africa (WB)", "Lebanon"] + entities.tolist(), index=0, key='entity_dropdown')
    ...
    year_slider = col1.slider('Year:', min_value=min_year, max_value=max_year, value=2019)
    ...
    def update_plot(year, entity):
        deaths_year_entity = sum_deaths[(sum_deaths['Year'] == year) & (sum_deaths['Entity'] == entity)].squeeze()
        ...
        bars = plt.barh(deaths_year_entity_sorted.index, deaths_year_entity_sorted.values, color='skyblue')
        ...
        with col_top:
            st.markdown("<h3 style='font-size: 50px;'>Top Risk Factors:</h3>", unsafe_allow_html=True)
            for i, category in enumerate(reversed(top_categories_names), 1):
                st.markdown(f'<p style="font-size: 35px;">{i}. {category}</p>', unsafe_allow_html=True)
...
def table_risk_comparison():
    ...
    styled_df = df.style.applymap(style_cell).set_properties(**{'text-align': 'left'}).set_caption("Top 6 Risk Factors")

    # Define the CSS properties for the table
    table_style = [
        {'selector': 'table', 'props': [('font-size', '40px'), ('background-color', '#F8F8F8')]},
...
def map_deathby_smok(col):
    ...
    option = st.radio('Select an option', ['Smoking per 100,000 people', 'Smoking by %'])

    # Filter the dataset based on user selection
    if selected_country == 'All Countries':
        filtered_data = mapdata[(mapdata['Year'] >= year_range[0]) &
                                (mapdata['Year'] <= year_range[1])]
    else:
        filtered_data = mapdata[(mapdata['Entity'] == selected_country) &
    ...
    fig = px.scatter_geo(filtered_data, lat='lat', lon='long', color=column_name,
                        hover_data=['Entity', column_name], projection='equirectangular',
...
def gender():
    ...
    filtered_data = gender_data[(gender_data['Year'] >= year_range[0]) & (gender_data['Year'] <= year_range[1])]
    ...
    def filter_countries(data, region):
        if region == 'mena':
            mena_countries = ['Bahrain', 'Cyprus', 'Egypt', 'Iran', 'Iraq', 'Israel', 'Jordan', 'Kuwait',
    ...
    agg_data['Circle Size'] = agg_data['Population (historical estimates)'] * circle_scaling_factor

    # Create a scatter plot using Matplotlib
    plt.figure(figsize=(12, 9))

    plt.scatter(agg_data['Prevalence of current tobacco use, females (% of female adults)'],
                agg_data['Prevalence of current tobacco use, males (% of male adults)'],
                s=agg_data['Circle Size'], alpha=0.7)

    # Add labels to the data points
    for i in range(len(agg_data)):
        plt.text(agg_data.iloc[i]['Prevalence of current tobacco use, females (% of female adults)'],
                 agg_data.iloc[i]['Prevalence of current tobacco use, males (% of male adults)'],
                 agg_data.iloc[i]['Entity'], fontsize=10, ha='center', va='center')
...
def barplot_female_smokers():
    ...
    mena_countries = ['Bahrain', 'Cyprus', 'Egypt', 'Iran', 'Iraq', 'Israel', 'Jordan', 'Kuwait',
                      'Lebanon', 'Libya', 'Morocco', 'Oman', 'Palestine', 'Qatar', 'Saudi Arabia',
                      'Syria', 'Tunisia', 'Turkey', 'United Arab Emirates', 'Yemen', 'World']
    ...
    fig = px.bar(data_2019, x='Entity', y='Prevalence of current tobacco use, females (% of female adults)',
                 title='Percentage of Female Smokers in 2019 - MENA Region and World',
                 labels={'Entity': 'Country', 'Prevalence of current tobacco use, females (% of female adults)': 'Percentage'})
...
def line_male_smokers():
    ...
    fig = px.line(data_years, x='Year', y='Prevalence of current tobacco use, males (% of male adults)',
                color='Entity',
                title='Percentage of Male Smokers Over Time - MENA Region and World',
                labels={'Year': 'Year', 'Prevalence of current tobacco use, males (% of male adults)': 'Percentage'})
    ...
    if button_increase:
        increased_countries = data_years.groupby('Entity')['Prevalence of current tobacco use, males (% of male adults)'].last() > data_years.groupby('Entity')['Prevalence of current tobacco use, males (% of male adults)'].first()
    ...
    if button_decrease:
        decreased_countries = data_years.groupby('Entity')['Prevalence of current tobacco use, males (% of male adults)'].last() < data_years.groupby('Entity')['Prevalence of current tobacco use, males (% of male adults)'].first()
...
def line_female_smokers():
    ...
    # Create a line chart using Plotly Express
    fig = px.line(data_years, x='Year', y='Prevalence of current tobacco use, females (% of female adults)',
                  color='Entity',
                  title='Percentage of Female Smokers Over Time - MENA Region and World',
                  labels={'Year': 'Year', 'Prevalence of current tobacco use, females (% of female adults)': 'Percentage'})
    ...
    if button_increase:
        increased_countries = data_years.groupby('Entity')['Prevalence of current tobacco use, females (% of female adults)'].last() > data_years.groupby('Entity')['Prevalence of current tobacco use, females (% of female adults)'].first()
        increased_countries = increased_countries[increased_countries].index.tolist()
        fig.data = [trace for trace in fig.data if trace.name in increased_countries]

    if button_decrease:
        decreased_countries = data_years.groupby('Entity')['Prevalence of current tobacco use, females (% of female adults)'].last() < data_years.groupby('Entity')['Prevalence of current tobacco use, females (% of female adults)'].first()
        decreased_countries = decreased_countries[decreased_countries].index.tolist()
        fig.data = [trace for trace in fig.data if trace.name in decreased_countries]
...
def income():
    ...
    st.header("Did smoking trends change over time between different income levels around the world?")

    deathsby = pd.read_csv("number-of-deaths-by-risk-factor.csv")
    
    # Filter the data by income categories and deaths by smoking
    income_levels = ['World Bank Low Income', 'World Bank Lower Middle Income', 'World Bank Upper Middle Income', 'World Bank High Income']
    deaths_by_smoking = deathsby[deathsby['Entity'].isin(income_levels) & (deathsby['Deaths - Cause: All causes - Risk: Smoking - Sex: Both - Age: All Ages (Number)'] > 0)]

    # Create a line plot using Plotly Express
    fig = px.line(deaths_by_smoking, x='Year', y='Deaths - Cause: All causes - Risk: Smoking - Sex: Both - Age: All Ages (Number)',
                  color='Entity',
                  labels={'Year': 'Year', 'Deaths - Cause: All causes - Risk: Smoking - Sex: Both - Age: All Ages (Number)': 'Number of Deaths'},
...
def line_mena_smok_deaths():
    ...
    mena_countries_data = data[data['Entity'].isin(['Bahrain', 'Egypt', 'Iraq',
    ...
    mena_countries_total_deaths = mena_countries_data.pivot_table(values='(Percent)',
                                                                  index='Year',
                                                                  columns='Entity',
    ...
    fig.add_trace(go.Scatter(x=world_total_deaths['Year'],
    ...
    for country in mena_countries_total_deaths.columns[1:]:
        fig.add_trace(go.Scatter(x=mena_countries_total_deaths['Year'],
    ...
    fig.update_layout(
        title=dict(
            ...
            font=dict(
                size=22  # Adjust the font size of the title
            )
        ),
        xaxis_title='Year',
        ...
        legend=dict(
            font=dict(
                size=22  # Adjust the font size of the legend
            )
        ),
        xaxis=dict(
            title=dict(
                font=dict(
                    size=22  # Adjust the font size of the x-axis label
                )
            ),
            tickfont=dict(
                size=22  # Adjust the font size of the x-axis tick labels
            )
        ),
        yaxis=dict(
            title=dict(
                font=dict(
                    size=22  # Adjust the font size of the y-axis label
                )
            ),
            tickfont=dict(
                size=22  # Adjust the font size of the y-axis tick labels
            )
        )
    )
    ...
    if button_increasing:
        filtered_countries = mena_countries_total_deaths.columns[1:][mena_countries_total_deaths.iloc[-1, 1:] > mena_countries_total_deaths.iloc[0, 1:]]
...
def affordability():
    ...
    fig, ax = plt.subplots()
    ax.bar(selected_data['Entity'], selected_data['Indicator:Affordability - percentage of GDP per capita required to purchase 2000 cigarettes of the most sold brand'])
    ax.set_title(f"% of GDP per capita to purchase 2000 cigarettes of popular brand ({selected_year})")
...
def affordability_table():
    ...
    st.markdown(
        "<p style='font-size:{}px; color:{};'>{}</p>".format(font_size, color_normal, info_text),
        unsafe_allow_html=True
    )

    for ranks in ranks:
        st.markdown(
            "<p style='font-size:{}px; font-weight:bold; color:{};'>{}</p>".format(font_size, color_bold, ranks),
            unsafe_allow_html=True
        )
...
def recommend():
    ...
    title_style = "background-color: maroon; color: white; font-weight: bold; padding: 8px;"
    ...
    for col, recommendation in zip(cols, recommendations):
        with col:
            st.markdown("<h4 style='font-size:{}px;{}'>{}</h4>".format(font_size, title_style, recommendation['title']), unsafe_allow_html=True)

            # Create an expander for the description
            with st.expander("Description", expanded=False):
                # Display the description
                st.markdown("<p style='font-size:{}px'>{}</p>".format(font_size, recommendation['description']), unsafe_allow_html=True)
...
def create_mind_map():
    ...
    nodes = {
            'Deaths attributed to smoking': ['  cancer       ', 'heart disease','   stroke        ','kidney disease',' lung disease ', ' miscarriage ' ],
    ...
    node_style = 'style="filled", fillcolor="#800000", fontcolor="white", fontweight="bold"'
...
def million_premature_deaths():
    ...
    st.markdown(
        "<p style='font-size:{}px; animation: highlight 2s infinite; color:{}; font-weight: bold;'>{}</p>".format(font_size, color, info_text),
        unsafe_allow_html=True
    )
...
def display_study_questions():
    ...
    for question in questions:
        st.markdown(
            "<p style='font-size:{}px; font-weight:bold; color:{};'>{}</p>".format(font_size, color_bold, question),
            unsafe_allow_html=True
        )
